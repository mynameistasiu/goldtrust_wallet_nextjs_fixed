...
export default function Withdraw() {
  ...
  useEffect(()=>{
    ...
    setRecentTx(recent.slice(0,3)); // show last 3
  },[]);
  ...
  const proceed = ()=>{
    ...
    setTimeout(()=>{
      ...
      setLoading(false);
      alert(`✅ Withdrawal request of ₦${amt} submitted!\nYou will receive it shortly.`);
  ...
  return (
    <Layout>
      ...
      <div className="card shadow-lg p-6 rounded-2xl space-y-4">
        ...
        <p className="text-gray-500 small">Your Wallet Balance: <b>₦{balance}</b></p>
            ...
            <ul className="space-y-1 text-sm text-gray-700">
              {recentTx.map((t,i)=>(
                <li key={i} className="flex justify-between">
                  <span>{t.bank} • {t.account}</span>
                  <span className={t.status==='pending'?'text-yellow-600':'text-green-600'}>₦{t.amount} • {t.status}</span>
                </li>
              ))}
            </ul>
          </div>
